...
$min_year = !empty($earliest_post) ? date('Y', strtotime($earliest_post[0]->post_date)) : date('Y');
$current_year = date('Y');

// 确定年份范围
$year_keys = !empty($years) ? array_keys($years) : [$current_year];
$max_year = max($current_year, max($year_keys));
$min_year = min($min_year, min($year_keys));
...
foreach ($years as $year => $months) {
    $postCount = empty($months) ? 0 : array_sum(array_map('count', $months));
